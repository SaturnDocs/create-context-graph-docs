...
for (const page of manifest.pages) {
  ...
  const htmlPath = path.join(build, relative, "index.html");
  const twinPath = path.join(build, `${relative}.md`);
...
for (const asset of manifest.assets.filter((entry) => entry.published)) {
  const built = await readFile(path.join(build, "img", path.basename(asset.source))).catch(() => null);
...
for (const page of manifest.pages) {
  const expected = `https://create-context-graph.saturndocs.net${page.route}/`;
...
if (failures.length > 0) {
  ...
  console.log(
    `Verified ${manifest.pageCount} rendered pages, ${manifest.pageCount} Markdown twins, ` +
      `${manifest.publishedAssetCount} byte-identical built assets, and complete sitemap coverage.`,
  );
}
